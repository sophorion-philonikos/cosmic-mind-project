...
const greenGlowTexture = generateGlowTexture(57, 255, 20); 
...
export default function StarMap({ 
  ...
  useEffect(() => {
    ...
    // FIX: A bulletproof shield. We explicitly verify both source AND target exist 
    // in the base universe to prevent the physics engine from crashing on hallucinated nodes.
    if (activeConnections && activeConnections.length > 0) {
      const validLinks = activeConnections.filter(link => {
        const sourceExists = baseUniverse.nodes.some((n: any) => n.id === link.source);
        const targetExists = baseUniverse.nodes.some((n: any) => n.id === link.target);
        return sourceExists && targetExists && activeChapterIds.includes(link.source) && activeChapterIds.includes(link.target);
    ...
    activeCommentaries.forEach(comm => {
      ...
      if (hostNode) {
        ...
        if (!existingSat && stableCoordsRef.current[hostNode.id]) {
            satelliteNode.x = stableCoordsRef.current[hostNode.id].x + (Math.random() * 10 - 5);
            satelliteNode.y = stableCoordsRef.current[hostNode.id].y + (Math.random() * 10 - 5);
            satelliteNode.z = stableCoordsRef.current[hostNode.id].z + (Math.random() * 10 - 5);
        }

        newUniverseNodes.push(satelliteNode);
      ...
      if (fgRef.current) {
        ...
        if (linkForce) {
          linkForce.distance((link: any) => {
            if (link.isSatelliteLink) return 25; 
            return link.isDynamic ? 60 : 30;
          });
          linkForce.strength((link: any) => {
            if (link.isSatelliteLink) return 1.2; 
        ...
        {normalizedActiveNodes.length > 0 && (
          <div className="flex flex-wrap gap-2 text-cyan-400 font-mono text-sm max-w-xl mt-2">
            ...
            {normalizedActiveNodes.map((node, idx) => {
              ...
              // Hide invalid citations from the HUD
              if (!baseUniverse.nodes.some((n: any) => n.id === targetId)) return null;
