...
export default function StarMap({ 
  ...
  useEffect(() => {
    selectedNodes.forEach(node => {
      // Don't fetch manuscript text if it's a commentary satellite (it already has the excerpt)
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (lastNode.isCommentary) return; // Disable keyboard nav for satellites
  ...
  useEffect(() => {
    ...
    // Inject Commentary Satellite Nodes
    activeCommentaries.forEach(comm => {
      newUniverseNodes.push({
        ...
        targetNodeId: comm.targetNodeId,
        // Spawn them slightly offset from origin so they drift naturally to target
    ...
    // Inject connections generated by the AI Oracle
    ...
    // Inject Cross References based on selection
    ...
    // Link Satellites to their primary texts
    activeCommentaries.forEach(comm => {
      if (baseUniverse.nodes.some((n: any) => n.id === comm.targetNodeId)) {
        newUniverseLinks.push({ source: comm.id, target: comm.targetNodeId, isSatelliteLink: true });
      }
    });
      ...
      if (fgRef.current) {
        ...
        if (linkForce) {
          linkForce.distance((link: any) => {
            if (link.isSatelliteLink) return 20; // Pull satellites very close
            return link.isDynamic ? 60 : 30;
          });
          linkForce.strength((link: any) => {
            if (link.isSatelliteLink) return 1.0; // High strength to keep orbit tight
  ...
  }, [JSON.stringify(activeConnections), JSON.stringify(normalizedActiveNodes), JSON.stringify(activeCommentaries), baseUniverse, isDynamicLayout, selectedNodes, crossRefDatabase, semanticIds, semanticLinks]); 
        ...
        <ForceGraph3D
          ...
          nodeThreeObject={(node: any) => {
            ...
            if (isHeatmapTarget) {
              ...
              const coreGeo = new THREE.SphereGeometry(2.5, 16, 16); // Slightly smaller
              const coreMat = new THREE.MeshBasicMaterial({ color: '#39ff14' }); // Neon Green
              const core = new THREE.Mesh(coreGeo, coreMat);
              group.add(core);
          ...
          linkColor={(link: any) => {
            if (link.isSatelliteLink) return 'rgba(57, 255, 20, 0.2)'; // Faint green tether
          ...
          linkWidth={(link: any) => {
            ...
            if (link.isSatelliteLink) return 0.5; // Thin tether
      ...
      {selectedNodes.length > 0 && (
        ...
        >
          {selectedNodes.map((node) => {
            ...
            // Render logic split: Commentary vs Primary Manuscript
            ...
            // Standard Primary Manuscript Render Logic
